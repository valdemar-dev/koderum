...
erase_line :: proc(params: [dynamic]int, index: int) {
    ...
    case 0:
        for c in terminal.cursor_col..<cell_count_x {
            buf[row_idx][c] = Cell{' ', terminal.current_fg_color, terminal.current_bg_color}
        }
    case 1:
...
set_graphics_rendition :: proc(params: [dynamic]int, index: int) {
    ...
    for i < len(params) {
        ...
        case 30..=37:
            color_idx := p - 30
            if terminal.current_bold {
                terminal.current_fg_color = ansi_bright_colors[color_idx]
            } else {
                terminal.current_fg_color = ansi_basic_colors[color_idx]
            }
        case 40..=47:
